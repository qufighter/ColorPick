...
//		},
	...
	browserAction: {
		setIcon: function(r){//UNIQUE
			widget.port.emit('setIconURI',r);//UNIQUE
		},
...
bgPage.port.on('request',function(request,sender,sendResponse){
	...
	//incomingRequest(request,sender,sendResponse);
	...
	//in incoming request function....  just send request to appropriate places...
	// can be handled inside  incomingRequest which will no longer send responses iteslf
});
...
bgPage.port.on('tab-request',function(request,sender,sendResponse){
	...
	var worker=findWorkerForCurrentTab();
	if(worker)worker.port.emit('request',request,{});
});
...
bgPage.port.on('response', function(response){
	...
	//firefoxChromeApi.response(response);//no longer accepting responses directly
});
...
pageMod.PageMod({
  ...
  onAttach: function(worker) {
    ...
    //worker.port.on('request',incomingRequest);//when bg page is really bg, these messages go to bg and popup (and options pages?) only
    ...
    worker.port.on('response', function(response){
    	...
    	//not so sure about this... seems okay though (see if we sent the request to the tab, this response might be for us too) 
    	//firefoxChromeApi.response(response);//when bg page is really bg, these will become defunct...
...
//		}
		...
		ss.storage['colorPickHistory']+="#"+curentHex;// <<<--------------- key
...
function handleRendering(){
	...
	var ox=Math.round(x);//(x/wid)*(wid-16);
	var oy=Math.round(y);//(y/hei)*(hei-16);
	sx=ox-startPoint;
	sy=oy-startPoint;
