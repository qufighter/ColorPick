...
popup.port.on('request',function(request,sender,sendResponse){
	...
	if(request.enableColorPicker){
		findWorkerForCurrentTab();
		if(workerHasChanged){
			request.workerHasChanged=true;
		}
	}
...
popup.port.on("connect", function(request) {
	currentWorker.port.emit('connect',request);
});
popup.port.on("disconnect", function(request) {
	currentWorker.port.emit('connect',request);
});

popup.port.on('storage-get-req', function(toGet){
	popup.port.emit('storage-get-resp',fromPrefs(toGet));
});
popup.port.on('storage-set-req', function(toSet){
	popup.port.emit('storage-set-resp',setPrefs(toSet));
});
...
pageMod.PageMod({
  ...
  attachTo: ["existing", "top"],
  contentScriptFile: [data.url('chrome-api.js'),data.url('options_prefs.js'),data.url('Cr_min.js'),data.url("colorpick.user.js"),data.url('colorpick.user-ff-extra.js')],
  ...
  onAttach: function(worker) {
    ...
    worker.port.on('getLocalization', function(response){
		...
		worker.port.on('storage-get-req', function(toGet){
			worker.port.emit('storage-get-resp',fromPrefs(toGet));
		});
		worker.port.on('storage-set-req', function(toSet){
			worker.port.emit('storage-set-resp',setPrefs(toSet));
		});
  }
});
...
function createPopupByEnum(popup_enum,url_extra){
	...
	optionsPanel.port.on('storage-get-req', function(toGet){
		optionsPanel.port.emit('storage-get-resp',fromPrefs(toGet));
	});
	optionsPanel.port.on('storage-set-req', function(toSet){
		optionsPanel.port.emit('storage-set-resp',setPrefs(toSet));
	});
